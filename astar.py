...
class CostMap(object):
    ...
    def obstacle_upload(self, obstacle_list):
        for obs in obstacle_list:
            self.map[obs[0]][obs[1]] = np.inf
...
def A_star_path_planning(cost_map, start_point, goal_point):
    ...
    while start_point not in Path:
        try:
            ...
            print("已回溯到起点")
            for p in CloseList:
                close_list.append(p.SelfPosition)
            return Path, close_list
...
# 路径规划
# path, close_list = A_star_path_planning(AstarMap.map, startPoint, endPoint)
